...
#
# boot_patch.sh      script        A script to patch boot image for IcePatch.
#                  (this file)      The script will use files in its same
#                                  directory to complete the patching process.
# bootimg            binary        The target boot image
# kpimg              binary        KernelPatch core Image
# kptools            executable    The KernelPatch tools binary (unpack/repack boot.img, inject kpimg).
#
#######################################################################################
...
echo "****************************"
echo " IcePatch Boot Image Patcher"
echo "****************************"
...
if [ ! $(./kptools -i kernel -f | grep CONFIG_KALLSYMS=y) ]; then
	...
	echo "- IcePatch requires CONFIG_KALLSYMS to be Enabled."
...
if [ ! $(./kptools -i kernel.ori -f | grep CONFIG_KALLSYMS_ALL=y) ]; then
	...
	echo "- IcePatch has patched but maybe your device won't boot."
